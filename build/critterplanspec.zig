...
fn seg_ducks(_arg_segs: *CxList(Segment), ch: *CxList(i64), pose: Pose, d_: i64) *CxList(Placed) {
    return b0: { const sg = cx_list_at(_arg_segs, cx_list_at(ch, d_)); break :b0 (if ((d_ >= safari_seg_reach())) cx_ll_empty(Placed) else (if (is_pond(sg.exit_creature)) place_ducks(_arg_segs, ch, pose, cx_new(MapperS{ .is_chain = true, .d_ = d_, .prev_len = @as(f64, @bitCast(@as(i64, 0))), .prev_angle = @as(f64, @bitCast(@as(i64, 0))), .prev_right = false, .prev_w = @as(f64, @bitCast(@as(i64, 0))) }), sg.length, 0) else cx_ll_empty(Placed))); };
}
...
fn behind_billboards(_arg_segs: *CxList(Segment), ch: *CxList(i64), pose: Pose, prev_idx: i64) *CxList(Placed) {
    return b0: { const pv = cx_list_at(_arg_segs, prev_idx); break :b0 cx_ll_concat(place_all_via(_arg_segs, ch, pose, prev_map(pv), (pv.width / @as(f64, @bitCast(@as(i64, 4611686018427387904)))), corner_critters(pv.exit_creature, pv.length, pv.exit_right, (pv.width / @as(f64, @bitCast(@as(i64, 4611686018427387904))))), 0), behind_ducks(_arg_segs, ch, pose, pv)); };
}

fn behind_ducks(_arg_segs: *CxList(Segment), ch: *CxList(i64), pose: Pose, pv: Segment) *CxList(Placed) {
    return (if (is_pond(pv.exit_creature)) place_ducks(_arg_segs, ch, pose, prev_map(pv), pv.length, 0) else cx_ll_empty(Placed));
}
...
fn reach_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ farm_seg_reach(), safari_seg_reach() });
}

fn reach_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 3, 5 });
}

fn segs() *CxList(Segment) {
    return cx_ll_of(Segment, &[_]Segment{ segment_at(0), segment_at(1), segment_at(2), segment_at(3), segment_at(4), segment_at(5) });
}

fn chain() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0, 1, 2, 3, 4, 5 });
}
...
fn farm_at(d_: i64) i64 {
    return cx_list_len(seg_farm(segs(), chain(), origin(), d_, @as(f64, @bitCast(@as(i64, 4611686018427387904)))));
}

fn safari_at(d_: i64) i64 {
    return cx_list_len(seg_safari(segs(), chain(), origin(), d_, @as(f64, @bitCast(@as(i64, 4611686018427387904)))));
}

fn cutoff_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (farm_at(0) > 0), (farm_at(2) > 0), (farm_at(3) == 0), (farm_at(4) == 0), (farm_at(5) == 0), (safari_at(3) > 0), (safari_at(4) > 0), (safari_at(5) == 0) });
}

fn cutoff_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true, true, true, true, true, true, true });
}

fn term_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(seg_safari(cx_ll_of(Segment, &[_]Segment{ segment_at(18) }), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0, @as(f64, @bitCast(@as(i64, 4611686018427387904))))) });
}

fn term_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0 });
}

fn term_flag_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (cx_list_len(seg_safari(cx_ll_of(Segment, &[_]Segment{ segment_at(0) }), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0, @as(f64, @bitCast(@as(i64, 4611686018427387904))))) > 0) });
}

fn term_flag_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true });
}

fn duck_count(i_: i64) i64 {
    return cx_list_len(seg_ducks(cx_ll_of(Segment, &[_]Segment{ segment_at(i_) }), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0));
}

fn duck_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (duck_count(12) > 0), (duck_count(0) == 0), (duck_count(4) == 0), (duck_count(18) == 0) });
}

fn duck_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true, true, true });
}

fn duck_reach_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(seg_ducks(segs(), chain(), origin(), 5)), cx_list_len(seg_ducks(cx_ll_of(Segment, &[_]Segment{ segment_at(12) }), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0)) });
}

fn duck_reach_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0, 6 });
}

fn order_got() *CxList(i64) {
    return b0: { const d0: i64 = cx_list_len(seg_billboards(segs(), chain(), origin(), 0)); break :b0 b1: { const f0: i64 = farm_at(0); break :b1 b2: { const s0: i64 = safari_at(0); break :b2 b3: { const k0: i64 = cx_list_len(seg_ducks(segs(), chain(), origin(), 0)); break :b3 cx_ll_of(i64, &[_]i64{ (d0 - ((f0 + s0) + k0)) }); }; }; }; };
}

fn order_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0 });
}
...
fn walk_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(walk_billboards(segs(), cx_ll_empty(i64), origin(), 0)), (cx_list_len(walk_billboards(one_seg(), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0)) - cx_list_len(seg_billboards(one_seg(), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0))), cx_list_len(walk_billboards(segs(), chain(), origin(), 6)) });
}

fn walk_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0, 0, 0 });
}

fn grow_got() *CxList(bool) {
    return b0: { const a_: i64 = cx_list_len(walk_billboards(segs(), chain(), origin(), 0)); break :b0 b1: { const b_: i64 = cx_list_len(walk_billboards(segs(), chain(), origin(), 1)); break :b1 b2: { const c_: i64 = cx_list_len(walk_billboards(segs(), chain(), origin(), 2)); break :b2 cx_ll_of(bool, &[_]bool{ (a_ >= b_), (b_ >= c_) }); }; }; };
}

fn grow_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true });
}

fn behind_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (cx_list_len(behind_billboards(segs(), chain(), origin(), 0)) >= 0), (cx_list_len(behind_ducks(segs(), chain(), origin(), segment_at(12))) > 0), (cx_list_len(behind_ducks(segs(), chain(), origin(), segment_at(0))) == 0) });
}
...
fn opening() void {
    return b0: { _ = cx_print_line(grade_ints("\x18\x12\x49\x15\x0d\x0f\x18\x14\x02", reach_got(), reach_want())); _ = cx_print_line(grade_bools("\x18\x12\x49\x18\x19\x0e\x10\x1c\x1c", cutoff_got(), cutoff_want())); _ = cx_print_line(grade_ints("\x18\x12\x49\x0e\x0d\x15\x1a\x02\x02", term_got(), term_want())); _ = cx_print_line(grade_bools("\x18\x12\x49\x0e\x1c\x17\x0f\x1d\x02", term_flag_got(), term_flag_want())); _ = cx_print_line(grade_bools("\x18\x12\x49\x16\x19\x18\x22\x02\x02", duck_got(), duck_want())); _ = cx_print_line(grade_ints("\x18\x12\x49\x16\x15\x0d\x0f\x18\x14", duck_reach_got(), duck_reach_want())); _ = cx_print_line(grade_ints("\x18\x12\x49\x10\x15\x16\x0d\x15\x02", order_got(), order_want())); _ = cx_print_line(grade_ints("\x18\x12\x49\x1b\x0f\x17\x22\x02\x02", walk_got(), walk_want())); _ = cx_print_line(grade_bools("\x18\x12\x49\x1d\x15\x10\x1b\x02\x02", grow_got(), grow_want())); _ = cx_print_line(grade_bools("\x18\x12\x49\x20\x0d\x14\x11\x12\x16", behind_got(), behind_want())); break :b0; };
}

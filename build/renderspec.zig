...
fn seg_list(i_: i64) *CxList(Segment) {
    return (if ((i_ >= cx_list_len(route()))) cx_ll_empty(Segment) else cx_ll_concat(cx_ll_of(Segment, &[_]Segment{ segment_at(i_) }), seg_list((i_ + 1))));
}
...
fn frame() Collected {
    return collect(segs(), 0, origin(), @as(f64, @bitCast(@as(i64, 4647503709213818880))), @as(f64, @bitCast(@as(i64, 0))), v_base(), @as(f64, @bitCast(@as(i64, 4647503709213818880))));
}

fn prev_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ prev_index(0), prev_index(1), prev_index(5) });
}

fn prev_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0, 0, 4 });
}

fn merge_got() *CxList(i64) {
    return b0: { const n_: i64 = cx_list_len(frame().order); break :b0 b1: { const parts: i64 = (cx_list_len(tree_items(frame().trees, 0)) + cx_list_len(tower_items(frame().towers, 0))); break :b1 b2: { const more: i64 = (cx_list_len(cow_items(frame().cows, 0)) + cx_list_len(cat_items(frame().cats, 0))); break :b2 b3: { const rest: i64 = (cx_list_len(truck_items(frame().truck)) + cx_list_len(rail_items(frame().rails, 0))); break :b3 cx_ll_of(i64, &[_]i64{ (n_ - ((parts + more) + rest)) }); }; }; }; };
}

fn merge_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0 });
}

fn present_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (cx_list_len(frame().trees) > 0), (cx_list_len(frame().towers) > 0), (cx_list_len(frame().cows) > 0), (cx_list_len(frame().rails) > 0), (cx_list_len(frame().order) > 0) });
}
...
fn cap_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (cx_list_len(frame().trees) <= max_vis_trees()), (cx_list_len(frame().towers) <= max_vis_towers()), (cx_list_len(frame().cows) <= max_vis_critters()), (cx_list_len(frame().cats) <= max_vis_cats()) });
}

fn cap_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true, true, true });
}

fn cap_values_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ max_vis_trees(), max_vis_towers(), max_vis_cats() });
}

fn cap_values_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 640, 16, 8 });
}

fn descending(xs: *CxList(Item), i_: i64) bool {
    var _tl_i = i_;
    while (true) {
        if (((_tl_i + 1) >= cx_list_len(xs))) { return true; } else { if (deeper_than(cx_list_at(xs, (_tl_i + 1)).fwd, cx_list_at(xs, _tl_i).fwd)) { return false; } else { { const _tj2_1 = (_tl_i + 1); _tl_i = _tj2_1; continue; } } }
    }
}

fn sorted_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ descending(frame().order, 0), descending(cx_ll_empty(Item), 0) });
}

fn sorted_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true });
}

fn cull_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ (frame().cull_seg >= 0), (frame().cull_size >= 0), (frame().cull_seg > 0) });
}

fn cull_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true, true });
}

fn seg_cull_got() *CxList(i64) {
    return b0: { const a_: i64 = seg_cull_count(segs(), cx_ll_of(i64, &[_]i64{ 0, 1, 2, 3, 4, 5 }), 3); break :b0 b1: { const b_: i64 = (cx_list_len(cx_list_at(segs(), 3).cows) + cx_list_len(cx_list_at(segs(), 3).pigs)); break :b1 cx_ll_of(i64, &[_]i64{ (a_ - b_), seg_cull_count(segs(), cx_ll_of(i64, &[_]i64{ 0, 1, 2, 3, 4, 5 }), 0), seg_cull_count(segs(), cx_ll_of(i64, &[_]i64{ 0, 1, 2, 3, 4, 5 }), 2), walk_seg_cull(segs(), cx_ll_empty(i64), 0) }); }; };
}

fn seg_cull_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0, 0, 0, 0 });
}

fn shrink_got() *CxList(bool) {
    return b0: { const long: i64 = walk_seg_cull(segs(), cx_ll_of(i64, &[_]i64{ 0, 1, 2, 3, 4, 5 }), 0); break :b0 b1: { const short: i64 = walk_seg_cull(segs(), cx_ll_of(i64, &[_]i64{ 0, 1, 2 }), 0); break :b1 cx_ll_of(bool, &[_]bool{ (long > short), (short == 0) }); }; };
}

fn shrink_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, true });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_ints("\x15\x12\x49\x1f\x15\x0d\x21\x02\x02", prev_got(), prev_want())); _ = cx_print_line(grade_ints("\x15\x12\x49\x1a\x0d\x15\x1d\x0d\x02", merge_got(), merge_want())); _ = cx_print_line(grade_bools("\x15\x12\x49\x1f\x15\x0d\x13\x12\x0e", present_got(), present_want())); _ = cx_print_line(grade_bools("\x15\x12\x49\x18\x0f\x1f\x02\x02\x02", cap_got(), cap_want())); _ = cx_print_line(grade_ints("\x15\x12\x49\x18\x0f\x1f\x13\x02\x02", cap_values_got(), cap_values_want())); _ = cx_print_line(grade_bools("\x15\x12\x49\x13\x10\x15\x0e\x0d\x16", sorted_got(), sorted_want())); _ = cx_print_line(grade_bools("\x15\x12\x49\x18\x19\x17\x17\x02\x02", cull_got(), cull_want())); _ = cx_print_line(grade_ints("\x15\x12\x49\x13\x0d\x1d\x18\x19\x17", seg_cull_got(), seg_cull_want())); _ = cx_print_line(grade_bools("\x15\x12\x49\x13\x14\x15\x11\x12\x22", shrink_got(), shrink_want())); break :b0; };
}

...
fn at(segs: *CxList(Segment), ch: *CxList(i64), _arg_pose: Pose, d_: i64, a_: f64, x: f64) RiderPt {
    return b0: { const p_ = compose_down(segs, ch, d_, a_, x); break :b0 to_rider(p_.a_, p_.x, _arg_pose.along, _arg_pose.across, _arg_pose.yaw, _arg_pose.hw); };
}

fn map_pt(segs: *CxList(Segment), ch: *CxList(i64), _arg_pose: Pose, m_: Mapper, a_: f64, x: f64) RiderPt {
    return (if (m_.is_chain) at(segs, ch, _arg_pose, m_.d_, a_, x) else b1: { const p_ = cur_to_next(a_, x, m_.prev_len, m_.prev_angle, m_.prev_right, m_.prev_w); break :b1 to_rider(p_.a_, p_.x, _arg_pose.along, _arg_pose.across, _arg_pose.yaw, _arg_pose.hw); });
}
...
fn bound_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ look_ahead(), max_chain() });
}

fn bound_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 7, 8 });
}

fn route_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(build_world()), cx_list_len(build_chain(build_world(), 0)), cx_list_len(build_chain(build_world(), 15)), cx_list_len(build_chain(build_world(), 18)) });
}

fn route_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 19, 7, 4, 1 });
}

fn pose() Pose {
    return cx_new(PoseS{ .along = @as(f64, @bitCast(@as(i64, 4636737291354636288))), .across = @as(f64, @bitCast(@as(i64, 4607182418800017408))), .yaw = @as(f64, @bitCast(@as(i64, 4596373779694328218))), .hw = @as(f64, @bitCast(@as(i64, 4611686018427387904))) });
}

fn m_chain() Mapper {
    return cx_new(MapperS{ .is_chain = true, .d_ = 3, .prev_len = @as(f64, @bitCast(@as(i64, 0))), .prev_angle = @as(f64, @bitCast(@as(i64, 0))), .prev_right = false, .prev_w = @as(f64, @bitCast(@as(i64, 0))) });
}

fn m_prev_left() Mapper {
    return cx_new(MapperS{ .is_chain = false, .d_ = 0, .prev_len = @as(f64, @bitCast(@as(i64, 4643985272004935680))), .prev_angle = @as(f64, @bitCast(@as(i64, 4608181004090891254))), .prev_right = false, .prev_w = @as(f64, @bitCast(@as(i64, 4616189618054758400))) });
}

fn m_prev_right() Mapper {
    return cx_new(MapperS{ .is_chain = false, .d_ = 0, .prev_len = @as(f64, @bitCast(@as(i64, 4643985272004935680))), .prev_angle = @as(f64, @bitCast(@as(i64, 4606035483714196905))), .prev_right = true, .prev_w = @as(f64, @bitCast(@as(i64, 4616189618054758400))) });
}

fn map_int_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ m_chain().d_, m_prev_left().d_ });
}

fn map_int_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 3, 0 });
}

fn map_real_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ m_chain().prev_len, m_chain().prev_angle, m_chain().prev_w, m_prev_left().prev_len, m_prev_left().prev_w });
}

fn map_real_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 4643985272004935680))), @as(f64, @bitCast(@as(i64, 4616189618054758400))) });
}

fn map_bool_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ m_chain().is_chain, m_chain().prev_right, m_prev_left().is_chain, m_prev_left().prev_right, m_prev_right().prev_right });
}

fn map_bool_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, false, false, false, true });
}

fn at_d0() RiderPt {
    return at(cx_ll_empty(Segment), cx_ll_empty(i64), pose(), 0, @as(f64, @bitCast(@as(i64, 4632233691727265792))), @as(f64, @bitCast(@as(i64, 4613937818241073152))));
}

fn at_got() *CxList(f64) {
    return b0: { const p_ = at(build_world(), build_chain(build_world(), 0), pose(), 1, @as(f64, @bitCast(@as(i64, 4632233691727265792))), @as(f64, @bitCast(@as(i64, 4613937818241073152)))); break :b0 cx_ll_of(f64, &[_]f64{ at_d0().right, at_d0().forward, p_.right, p_.forward }); };
}

fn at_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4621781662580624546))), (-@as(f64, @bitCast(@as(i64, 4632093422738818287)))), (-@as(f64, @bitCast(@as(i64, 4631968572044192773)))), @as(f64, @bitCast(@as(i64, 4646306676751031139))) });
}

fn behind_got() *CxList(f64) {
    return b0: { const l_ = map_pt(cx_ll_empty(Segment), cx_ll_empty(i64), pose(), m_prev_left(), @as(f64, @bitCast(@as(i64, 4632233691727265792))), @as(f64, @bitCast(@as(i64, 4613937818241073152)))); break :b0 b1: { const r_ = map_pt(cx_ll_empty(Segment), cx_ll_empty(i64), pose(), m_prev_right(), @as(f64, @bitCast(@as(i64, 4632233691727265792))), @as(f64, @bitCast(@as(i64, 4613937818241073152)))); break :b1 cx_ll_of(f64, &[_]f64{ l_.right, l_.forward, r_.right, r_.forward }); }; };
}

fn behind_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ (-@as(f64, @bitCast(@as(i64, 4641056549477685634)))), (-@as(f64, @bitCast(@as(i64, 4642353925966579420)))), @as(f64, @bitCast(@as(i64, 4642647857711599902))), (-@as(f64, @bitCast(@as(i64, 4641878880200968479)))) });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_ints("\x1c\x15\x49\x20\x10\x19\x12\x16", bound_got(), bound_want())); _ = cx_print_line(grade_ints("\x1c\x15\x49\x15\x10\x19\x0e\x0d", route_got(), route_want())); _ = cx_print_line(grade_ints("\x1c\x15\x49\x1a\x0f\x1f\x11\x02", map_int_got(), map_int_want())); _ = cx_print_line(grade_reals("\x1c\x15\x49\x1a\x0f\x1f\x15\x02", map_real_got(), map_real_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_bools("\x1c\x15\x49\x1a\x0f\x1f\x20\x02", map_bool_got(), map_bool_want())); _ = cx_print_line(grade_rel("\x1c\x15\x49\x0f\x0e\x02\x02\x02", at_got(), at_want(), @as(f64, @bitCast(@as(i64, 4497644614860975944))))); _ = cx_print_line(grade_rel("\x1c\x15\x49\x20\x0d\x14\x11\x12\x16", behind_got(), behind_want(), @as(f64, @bitCast(@as(i64, 4502148214488346440))))); break :b0; };
}

...
fn tower_if_ahead(_arg_segs: *CxList(Segment), ch: *CxList(i64), pose: Pose, m_: Mapper, a0: f64, x0: f64, yw: f64, key: i64) *CxList(TowerItem) {
    return b0: { const c_ = map_pt(_arg_segs, ch, pose, m_, a0, x0); break :b0 (if ((c_.forward <= near())) cx_ll_empty(TowerItem) else cx_ll_of(TowerItem, &[_]TowerItem{ cx_new(TowerItemS{ .map = m_, .a0 = a0, .x0 = x0, .yaw = yw, .fwd = c_.forward, .off_ = beacon_offset_for(key) }) })); };
}
...
fn behind_tower(_arg_segs: *CxList(Segment), ch: *CxList(i64), pose: Pose, prev_idx: i64) *CxList(TowerItem) {
    return b0: { const pv = cx_list_at(_arg_segs, prev_idx); break :b0 tower_if_ahead(_arg_segs, ch, pose, cx_new(MapperS{ .is_chain = false, .d_ = 0, .prev_len = pv.length, .prev_angle = pv.exit_angle, .prev_right = pv.exit_right, .prev_w = pv.width }), (pv.length + tower_beyond()), ((pv.width / @as(f64, @bitCast(@as(i64, 4611686018427387904)))) + tower_right()), tower_yaw(), prev_idx); };
}

fn all_towers(_arg_segs: *CxList(Segment), ch: *CxList(i64), pose: Pose, seg_idx: i64) *CxList(TowerItem) {
    return (if ((seg_idx > 0)) cx_ll_concat(walk_towers(_arg_segs, ch, pose, 0), behind_tower(_arg_segs, ch, pose, (seg_idx - 1))) else walk_towers(_arg_segs, ch, pose, 0));
}
...
fn fixed_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ tower_beyond(), tower_right(), seg_tower_left() });
}

fn fixed_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4639833516098453504))), @as(f64, @bitCast(@as(i64, 4626322717216342016))), @as(f64, @bitCast(@as(i64, 4636737291354636288))) });
}

fn cap_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ max_vis_towers() });
}

fn cap_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 16 });
}
...
fn short_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(walk_towers(segs(), chain(), origin(), 0)), cx_list_len(seg_towers(segs(), chain(), origin(), 0)), cx_list_len(seg_mid_tower(segs(), chain(), origin(), 0)), cx_list_len(all_towers(segs(), chain(), origin(), 0)) });
}

fn short_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 3, 1, 0, 3 });
}

fn mid_flags_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ cx_list_at(segs(), 0).has_mid_tower, cx_list_at(segs(), 1).has_mid_tower, cx_list_at(segs(), 2).has_mid_tower });
}

fn mid_flags_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ false, false, false });
}

fn where_got() *CxList(f64) {
    return b0: { const t = cx_list_at(seg_towers(segs(), chain(), origin(), 0), 0); break :b0 b1: { const u_ = cx_list_at(seg_towers(segs(), chain(), origin(), 1), 0); break :b1 cx_ll_of(f64, &[_]f64{ t.a0, t.x0, u_.a0, u_.x0 }); }; };
}

fn where_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4649016637213638656))), @as(f64, @bitCast(@as(i64, 4626885667169763328))), @as(f64, @bitCast(@as(i64, 4647151865492930560))), @as(f64, @bitCast(@as(i64, 4626885667169763328))) });
}

fn long_segs() *CxList(Segment) {
    return cx_ll_of(Segment, &[_]Segment{ segment_at(6) });
}

fn long_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(walk_towers(long_segs(), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0)), cx_list_len(seg_mid_tower(long_segs(), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0)) });
}

fn long_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 2, 1 });
}

fn long_where_got() *CxList(f64) {
    return b0: { const ts = walk_towers(long_segs(), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0); break :b0 cx_ll_of(f64, &[_]f64{ cx_list_at(ts, 0).a0, cx_list_at(ts, 0).x0, cx_list_at(ts, 1).a0, cx_list_at(ts, 1).x0, cx_list_at(ts, 0).yaw, cx_list_at(ts, 1).yaw }); };
}

fn long_where_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4653696158701453312))), @as(f64, @bitCast(@as(i64, 4626885667169763328))), @as(f64, @bitCast(@as(i64, 4648488871632306176))), (-@as(f64, @bitCast(@as(i64, 4636596553866280960)))), @as(f64, @bitCast(@as(i64, 4602891378041239713))), @as(f64, @bitCast(@as(i64, 0))) });
}

fn key_got() *CxList(f64) {
    return b0: { const ts = walk_towers(long_segs(), cx_ll_of(i64, &[_]i64{ 0 }), origin(), 0); break :b0 cx_ll_of(f64, &[_]f64{ cx_list_at(ts, 0).off_, cx_list_at(ts, 1).off_, beacon_offset_for(0), beacon_offset_for(60) }); };
}

fn key_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 4633641066610819072))), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 4633641066610819072))) });
}

fn spread_got() *CxList(f64) {
    return b0: { const ts = walk_towers(segs(), chain(), origin(), 0); break :b0 cx_ll_of(f64, &[_]f64{ cx_list_at(ts, 0).off_, cx_list_at(ts, 1).off_, cx_list_at(ts, 2).off_ }); };
}

fn spread_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 4630404104378646528))), @as(f64, @bitCast(@as(i64, 4634907704006017024))) });
}

fn behind_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(all_towers(segs(), chain(), origin(), 0)), cx_list_len(all_towers(segs(), chain(), origin(), 1)), cx_list_len(all_towers(segs(), chain(), origin(), 2)), cx_list_len(behind_tower(segs(), chain(), origin(), 0)) });
}

fn behind_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 3, 4, 4, 1 });
}

fn cull_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(walk_towers(segs(), chain(), cx_new(PoseS{ .along = @as(f64, @bitCast(@as(i64, 4662219572839972864))), .across = @as(f64, @bitCast(@as(i64, 0))), .yaw = @as(f64, @bitCast(@as(i64, 0))), .hw = @as(f64, @bitCast(@as(i64, 4611686018427387904))) }), 0)), cx_list_len(walk_towers(segs(), cx_ll_empty(i64), origin(), 0)) });
}
...
fn items_got() *CxList(i64) {
    return b0: { const ts = walk_towers(segs(), chain(), origin(), 0); break :b0 b1: { const its = tower_items(ts, 0); break :b1 cx_ll_of(i64, &[_]i64{ cx_list_len(its), cx_list_at(its, 0).i_, cx_list_at(its, 2).i_, cx_list_len(tower_items(cx_ll_empty(TowerItem), 0)) }); }; };
}

fn items_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 3, 0, 2, 0 });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_reals("\x0e\x17\x49\x1c\x11\x24\x0d\x16\x02", fixed_got(), fixed_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_ints("\x0e\x17\x49\x18\x0f\x1f\x02\x02\x02", cap_got(), cap_want())); _ = cx_print_line(grade_ints("\x0e\x17\x49\x13\x14\x10\x15\x0e\x02", short_got(), short_want())); _ = cx_print_line(grade_bools("\x0e\x17\x49\x1a\x11\x16\x02\x02\x02", mid_flags_got(), mid_flags_want())); _ = cx_print_line(grade_reals("\x0e\x17\x49\x1b\x14\x0d\x15\x0d\x02", where_got(), where_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_ints("\x0e\x17\x49\x17\x10\x12\x1d\x02\x02", long_got(), long_want())); _ = cx_print_line(grade_reals("\x0e\x17\x49\x17\x1b\x14\x0d\x15\x0d", long_where_got(), long_where_want(), @as(f64, @bitCast(@as(i64, 4427486594234968593))))); _ = cx_print_line(grade_reals("\x0e\x17\x49\x22\x0d\x1e\x02\x02\x02", key_got(), key_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_reals("\x0e\x17\x49\x13\x1f\x15\x0d\x0f\x16", spread_got(), spread_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_ints("\x0e\x17\x49\x20\x0d\x14\x11\x12\x16", behind_got(), behind_want())); _ = cx_print_line(grade_ints("\x0e\x17\x49\x18\x19\x17\x17\x02\x02", cull_got(), cull_want())); _ = cx_print_line(grade_ints("\x0e\x17\x49\x11\x0e\x0d\x1a\x13\x02", items_got(), items_want())); break :b0; };
}

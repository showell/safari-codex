...
fn simulate_rider_step(s_: RiderState, tilt_step: f64, accel: f64) RiderState {
    return b0: { const tilt: f64 = (s_.tilt + tilt_step); break :b0 b1: { const v_: f64 = (s_.v_ + accel); break :b1 b2: { const heading_change: f64 = (yaw_per_tilt() * tilt); break :b2 b3: { const mid: f64 = (s_.yaw + (heading_change / @as(f64, @bitCast(@as(i64, 4611686018427387904))))); break :b3 cx_new(RiderStateS{ .segment = s_.segment, .along = (s_.along + (v_ * real_cos(mid))), .across = (s_.across + (v_ * real_sin(mid))), .yaw = (s_.yaw + heading_change), .v_ = v_, .tilt = tilt, .heading = (s_.heading + heading_change), .gaze_yaw = s_.gaze_yaw, .focus = s_.focus }); }; }; }; };
}
...
fn limit_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ yaw_per_tilt(), max_lean() });
}

fn limit_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4591870180066957722))), @as(f64, @bitCast(@as(i64, 4599959831253042311))) });
}
...
fn s1() RiderState {
    return simulate_rider_step(s0(), @as(f64, @bitCast(@as(i64, 4587366580439587226))), @as(f64, @bitCast(@as(i64, 4576918229304087675))));
}

fn s2() RiderState {
    return simulate_rider_step(s1(), (@as(f64, @bitCast(@as(i64, 0))) - @as(f64, @bitCast(@as(i64, 4581421828931458171)))), @as(f64, @bitCast(@as(i64, 4572414629676717179))));
}

fn s3() RiderState {
    return simulate_rider_step(s2(), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 0))));
}

fn exact_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ s1().yaw, s1().v_, s1().tilt, s1().heading, s2().yaw, s2().v_, s2().tilt, s2().heading, s3().yaw, s3().v_, s3().tilt, s3().heading });
}

fn exact_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4600742271332877599))), @as(f64, @bitCast(@as(i64, 4605471050941616620))), @as(f64, @bitCast(@as(i64, 4595473059768854118))), @as(f64, @bitCast(@as(i64, 4616772271256549458))), @as(f64, @bitCast(@as(i64, 4601021494509774569))), @as(f64, @bitCast(@as(i64, 4605516086937890325))), @as(f64, @bitCast(@as(i64, 4594752483828474839))), @as(f64, @bitCast(@as(i64, 4616789722705105519))), @as(f64, @bitCast(@as(i64, 4601300717686671540))), @as(f64, @bitCast(@as(i64, 4605516086937890325))), @as(f64, @bitCast(@as(i64, 4594752483828474839))), @as(f64, @bitCast(@as(i64, 4616807174153661580))) });
}

fn carried_real_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ s1().gaze_yaw, s1().focus, s3().gaze_yaw, s3().focus });
}

fn carried_real_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4603804719079489536))), @as(f64, @bitCast(@as(i64, 4598175219545276416))), @as(f64, @bitCast(@as(i64, 4603804719079489536))), @as(f64, @bitCast(@as(i64, 4598175219545276416))) });
}

fn carried_int_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ s1().segment, s2().segment, s3().segment });
}

fn carried_int_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 3, 3, 3 });
}

fn pos_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ s1().along, s1().across, s2().along, s2().across, s3().along, s3().across });
}

fn pos_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4638232703616839588))), @as(f64, @bitCast(@as(i64, 4612931858829871625))), @as(f64, @bitCast(@as(i64, 4638285521364801273))), @as(f64, @bitCast(@as(i64, 4613646948496982302))), @as(f64, @bitCast(@as(i64, 4638337986410469631))), @as(f64, @bitCast(@as(i64, 4614388148819672407))) });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_reals("\x20\x22\x49\x17\x11\x1a\x11\x0e", limit_got(), limit_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_reals("\x20\x22\x49\x0d\x24\x0f\x18\x0e", exact_got(), exact_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_reals("\x20\x22\x49\x18\x0f\x15\x15\x1e", carried_real_got(), carried_real_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_ints("\x20\x22\x49\x13\x0d\x1d\x02\x02", carried_int_got(), carried_int_want())); _ = cx_print_line(grade_rel("\x20\x22\x49\x1f\x10\x13\x02\x02", pos_got(), pos_want(), @as(f64, @bitCast(@as(i64, 4472406533629990549))))); break :b0; };
}

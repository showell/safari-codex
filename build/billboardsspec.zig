...
fn fwd_in() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ (-@as(f64, @bitCast(@as(i64, 4617315517961601024)))), @as(f64, @bitCast(@as(i64, 4599075939470750515))), @as(f64, @bitCast(@as(i64, 4600877379321698714))), @as(f64, @bitCast(@as(i64, 4602678819172646912))), @as(f64, @bitCast(@as(i64, 4643985272004935680))), @as(f64, @bitCast(@as(i64, 4644724143818801152))), @as(f64, @bitCast(@as(i64, 4644741736004845568))), @as(f64, @bitCast(@as(i64, 4645744490609377280))), @as(f64, @bitCast(@as(i64, 4621819117588971520))), @as(f64, @bitCast(@as(i64, 4632233691727265792))) });
}

fn h_in() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 4606281698874543309))) });
}

fn placed_at(i_: i64) Placed {
    return b0: { const p_ = cx_new(RiderPtS{ .right = @as(f64, @bitCast(@as(i64, 4612811918334230528))), .forward = cx_list_at(fwd_in(), i_) }); break :b0 verdict(p_, cx_list_at(h_in(), i_), 128004, true); };
}

fn placed_all(i_: i64) *CxList(Placed) {
    return (if ((i_ >= cx_list_len(fwd_in()))) cx_ll_empty(Placed) else cx_ll_concat(cx_ll_of(Placed, &[_]Placed{ placed_at(i_) }), placed_all((i_ + 1))));
}

fn kept_got() *CxList(bool) {
    return list_map(Placed, bool, b1: { const _Env1 = struct { fn call(_ctx1: *anyopaque, _cp0: Placed) bool { _ = _ctx1; return __lam_0(_cp0); } }; break :b1 CxFn1(Placed, bool){ .ctx = cx_new(_Env1{  }), .call = &_Env1.call }; }, placed_all(0));
}

fn kept_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ false, false, false, true, true, true, false, false, false, true });
}

fn culled_got() *CxList(bool) {
    return list_map(Placed, bool, b1: { const _Env1 = struct { fn call(_ctx1: *anyopaque, _cp0: Placed) bool { _ = _ctx1; return __lam_1(_cp0); } }; break :b1 CxFn1(Placed, bool){ .ctx = cx_new(_Env1{  }), .call = &_Env1.call }; }, placed_all(0));
}

fn culled_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ false, false, false, false, false, false, true, true, true, false });
}

fn carried_real_got() *CxList(f64) {
    return b0: { const k_ = placed_at(3); break :b0 b1: { const n_ = placed_at(1); break :b1 cx_ll_of(f64, &[_]f64{ k_.b_.right, k_.b_.fwd, k_.b_.height, n_.b_.right, n_.b_.fwd, n_.b_.height }); }; };
}

fn carried_real_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4612811918334230528))), @as(f64, @bitCast(@as(i64, 4602678819172646912))), @as(f64, @bitCast(@as(i64, 4607182418800017408))), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 0))), @as(f64, @bitCast(@as(i64, 0))) });
}

fn carried_int_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ placed_at(3).b_.cp_, placed_at(1).b_.cp_ });
}

fn carried_int_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 128004, 0 });
}

fn carried_bool_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ placed_at(3).b_.face_right, placed_at(1).b_.face_right });
}

fn carried_bool_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, false });
}

fn harvest_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ cx_list_len(kept_of(placed_all(0), 0)), size_culled_of(placed_all(0), 0), cx_list_len(placed_all(0)) });
}

fn harvest_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 4, 3, 10 });
}

fn floor_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ min_critter_px() });
}

fn floor_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4611686018427387904))) });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_bools("\x20\x20\x49\x22\x0d\x1f\x0e\x02", kept_got(), kept_want())); _ = cx_print_line(grade_bools("\x20\x20\x49\x18\x19\x17\x17\x0d\x16", culled_got(), culled_want())); _ = cx_print_line(grade_reals("\x20\x20\x49\x18\x0f\x15\x15\x1e", carried_real_got(), carried_real_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_ints("\x20\x20\x49\x18\x1f\x02\x02\x02", carried_int_got(), carried_int_want())); _ = cx_print_line(grade_bools("\x20\x20\x49\x1c\x0f\x18\x0d\x02", carried_bool_got(), carried_bool_want())); _ = cx_print_line(grade_ints("\x20\x20\x49\x18\x10\x19\x12\x0e", harvest_got(), harvest_want())); _ = cx_print_line(grade_reals("\x20\x20\x49\x1c\x17\x10\x10\x15", floor_got(), floor_want(), @as(f64, @bitCast(@as(i64, 0))))); break :b0; };
}

fn __lam_0(p_: Placed) bool {
    return p_.kept;
}

fn __lam_1(p_: Placed) bool {
    return p_.size_culled;
}

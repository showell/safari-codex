...
fn road_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ lane_width(), herd_road_offset() });
}

fn road_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4616189618054758400))), @as(f64, @bitCast(@as(i64, 4621819117588971520))) });
}

fn pond_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ is_pond(Creature.DuckPond), is_pond(Creature.NoCreature), is_pond(Creature.Elephant), is_pond(Creature.Giraffe), is_pond(Creature.Zebra), is_pond(Creature.Rhino) });
}

fn pond_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true, false, false, false, false, false });
}

fn scheme_code(s_: Scheme) i64 {
    return @as(i64, switch (s_) { .AllGreen => 0, .YellowGreen => 1, .RedGreen => 2,  });
}

fn creature_code(c_: Creature) i64 {
    return @as(i64, switch (c_) { .NoCreature => 0, .Elephant => 1, .Giraffe => 2, .Zebra => 3, .Rhino => 4, .DuckPond => 5,  });
}

fn code_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ scheme_code(Scheme.AllGreen), scheme_code(Scheme.YellowGreen), scheme_code(Scheme.RedGreen), creature_code(Creature.NoCreature), creature_code(Creature.Elephant), creature_code(Creature.Giraffe), creature_code(Creature.Zebra), creature_code(Creature.Rhino), creature_code(Creature.DuckPond) });
}

fn code_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 0, 1, 2, 0, 1, 2, 3, 4, 5 });
}

fn a_tree() Tree {
    return cx_new(TreeS{ .along = @as(f64, @bitCast(@as(i64, 4638179850610278400))), .across = @as(f64, @bitCast(@as(i64, 4614500768194494464))), .color = 9066271, .height = @as(f64, @bitCast(@as(i64, 4620411742705418240))) });
}

fn a_critter() Critter {
    return cx_new(CritterS{ .along = @as(f64, @bitCast(@as(i64, 4643017701772492800))), .across = (-@as(f64, @bitCast(@as(i64, 4619004367821864960)))), .codepoint = 128004, .height = @as(f64, @bitCast(@as(i64, 4608871268660281344))), .face_right = true });
}

fn rec_reals_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ a_tree().along, a_tree().across, a_tree().height, a_critter().along, a_critter().across, a_critter().height });
}

fn rec_reals_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4638179850610278400))), @as(f64, @bitCast(@as(i64, 4614500768194494464))), @as(f64, @bitCast(@as(i64, 4620411742705418240))), @as(f64, @bitCast(@as(i64, 4643017701772492800))), (-@as(f64, @bitCast(@as(i64, 4619004367821864960)))), @as(f64, @bitCast(@as(i64, 4608871268660281344))) });
}

fn rec_ints_got() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ a_tree().color, a_critter().codepoint });
}

fn rec_ints_want() *CxList(i64) {
    return cx_ll_of(i64, &[_]i64{ 9066271, 128004 });
}

fn rec_bools_got() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ a_critter().face_right });
}

fn rec_bools_want() *CxList(bool) {
    return cx_ll_of(bool, &[_]bool{ true });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_reals("\x13\x18\x49\x15\x10\x0f\x16", road_got(), road_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_bools("\x13\x18\x49\x1f\x10\x12\x16", pond_got(), pond_want())); _ = cx_print_line(grade_ints("\x13\x18\x49\x18\x10\x16\x0d", code_got(), code_want())); _ = cx_print_line(grade_reals("\x13\x18\x49\x15\x0d\x18\x15", rec_reals_got(), rec_reals_want(), @as(f64, @bitCast(@as(i64, 0))))); _ = cx_print_line(grade_ints("\x13\x18\x49\x15\x0d\x18\x11", rec_ints_got(), rec_ints_want())); _ = cx_print_line(grade_bools("\x13\x18\x49\x15\x0d\x18\x20", rec_bools_got(), rec_bools_want())); break :b0; };
}

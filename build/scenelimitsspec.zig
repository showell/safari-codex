...
fn culls_got() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ detail_dist(), crown_shade_dist(), min_scenery_px() });
}

fn culls_want() *CxList(f64) {
    return cx_ll_of(f64, &[_]f64{ @as(f64, @bitCast(@as(i64, 4641240890982006784))), @as(f64, @bitCast(@as(i64, 4635329916471083008))), @as(f64, @bitCast(@as(i64, 4611686018427387904))) });
}

fn opening() void {
    return b0: { _ = cx_print_line(grade_reals("\x13\x17\x49\x18\x19\x17\x17", culls_got(), culls_want(), @as(f64, @bitCast(@as(i64, 0))))); break :b0; };
}
